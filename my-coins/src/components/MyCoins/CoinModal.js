...
import { ModalFooter, Modal, ModalBody, Form, Button, Col, Row, CardHeader } from 'reactstrap';
import ClientInput from '../Common/ClientInput';

const CoinModal = ({ collapseOne, collapseOneOpen, saveDataInList }) => {
  const toggle = () => {
    ...
    <>
      <Modal isOpen={collapseOne} size="lg" scrollable={true}>
        ...
        <ModalBody className="bg-light">
          <Form>
            <ClientInput id="Name" label="Name" value={coinName} onChange={({ target }) => setCoinName(target.value)} />

            <ClientInput id="Ticker" label="Ticker" value={ticker} onChange={({ target }) => setTicker(target.value)} />
            <ClientInput id="Price" label="Price" value={price} onChange={({ target }) => setPrice(target.value)} />
            <ClientInput
              id="PriceChange"
              label="PriceChange"
              value={priceChange}
          ...
          {error && <div style={{ color: 'red' }}>Please enter all the values, they are required!</div>}
        </ModalBody>
        <ModalFooter className="justify-content-between bg-light">
          <Button color="secondary" size="sm" className="shadow" onClick={toggle}>
            Cancel
          </Button>
          <Button color="primary" size="sm" className="shadow" onClick={handleSubmit}>
            Save
          </Button>
        </ModalFooter>
      </Modal>
